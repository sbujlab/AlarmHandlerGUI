...
class ALARM_HANDLER(tk.Frame):
  ...
  def initialize_buttons(self,objectList):
    ...
    for i in range(0, 4):
      ...
      for j in range(0,len(row)):
        if i==3:
          row[j].grid(row=j,column=i,columnspan=2,padx=10,pady=10,sticky='N')
        else:
          row[j].grid(row=j,column=i,padx=10,pady=10,sticky='N')
  ...
  def select_add_button(self,objectList,but):
    ...
    if i==3:
      self.buttons[i][j].grid(row=j,column=i,columnspan=2,pady=10,padx=10,sticky='N')
    else:
      self.buttons[i][j].grid(row=j,column=i,pady=10,padx=10,sticky='N')
    self.alarmColumns[i].grid(row=0,column=i,pady=10,padx=10,sticky='N')
